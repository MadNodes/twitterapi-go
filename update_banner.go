// Doc https://docs.twitterapi.io/api-reference/endpoint/update_banner_v2
...
func (t *TwitterApi) UpdateBanner(filename string, filebody io.Reader) error {
	...
	if err != nil {
		slog.Error("UpdateBanner failed", "err", err)
		return err
	}
	...
	req, err := http.NewRequestWithContext(ctx1, http.MethodPatch, twitterDomainURI+"/update_banner_v2", body)
	if err != nil {
		slog.Error("UpdateBanner failed", "err", err)
		return err
	}
	...
	if resp.StatusCode != http.StatusOK {
		slog.Error("UpdateBanner failed", "statusCode", resp.StatusCode, "body", string(jsonData))
		return errors.New("UpdateBanner failed")
	}

	response := &updateBannerResponse{}
	if err = jsoniter.Unmarshal(jsonData, &response); err != nil {
		slog.Error("UpdateBanner failed", "err", err)
		return err
	}
	if response.Status != "success" {
		slog.Error("UpdateBanner failed", "status", response.Status, "message", response.Message)
		return errors.New("UpdateBanner failed")
	}

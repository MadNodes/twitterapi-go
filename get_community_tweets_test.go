...
func TestGetCommunityTweets(t *testing.T) {
	client := newTestClient(t)
	url := twitterDomainURI + "/community/tweets?community_id=" + testCommunityID

	raw, statusCode, err := doGet(t, client, url)
	if err != nil {
		t.Fatalf("GetCommunityTweets request failed: %v", err)
	}
	t.Logf("RAW JSON: %s", string(raw))
	if statusCode/100 != 2 {
		t.Fatalf("GetCommunityTweets returned non-2xx status: %d", statusCode)
	}

	var response GetCommunityTweetsResponse
...
func TestGetCommunityTweets_Behavior(t *testing.T) {
	...
	t.Run("empty communityID returns error", func(t *testing.T) {
		x := New(apiKey)
		_, err := x.GetCommunityTweets("", nil)
		if err == nil {
	...
	t.Run("valid communityID returns data", func(t *testing.T) {
		x := New(apiKey)
		resp, err := x.GetCommunityTweets("1512879283559141381", nil)
		if err != nil {

...
func TestGetTweetRetweeter(t *testing.T) {
	client := newTestClient(t)
	url := twitterDomainURI + "/tweet/retweeters?tweetId=" + testTweetID

	raw, statusCode, err := doGet(t, client, url)
	if err != nil {
		t.Fatalf("GetTweetRetweeter request failed: %v", err)
	}
	t.Logf("RAW JSON: %s", string(raw))
	if statusCode/100 != 2 {
		t.Fatalf("GetTweetRetweeter returned non-2xx status: %d", statusCode)
	}

	var response GetTweetRetweeterResponse
...
func TestGetTweetRetweeter_Behavior(t *testing.T) {
	...
	t.Run("valid tweetID returns data", func(t *testing.T) {
		x := New(apiKey)
		resp, err := x.GetTweetRetweeter(testTweetID, nil)
		if err != nil {
